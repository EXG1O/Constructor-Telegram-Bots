""""
Скрипт для того, чтобы сделать скрины всех страниц на сайте.
Скрипт не будет работать без драйвера "selenium" для Chrome.
Можно удалить, если вам он не нужен!
"""
...
for page in initial_pages:
	...
	time.sleep(1)
	browser.save_screenshot(f'site_pages_images/{page}_page.jpg')
...
bot_link.click()
time.sleep(1)
browser.save_screenshot(f'site_pages_images/view_bot_konstruktor_page.jpg')

add_command_button = browser.find_element_by_class_name('add-command-button-control')
add_command_button.click()
time.sleep(1)
browser.save_screenshot(f'site_pages_images/add_command_page.jpg')
