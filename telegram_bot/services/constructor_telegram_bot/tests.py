...
class ConstructorTelegramBotTests(BaseTestCase):
	...
	async def test_start_command_handler(self) -> None:
		methods: list[TelegramMethod[Any]] | None = await self.send_message('/start')

		assert methods is not None

		method: TelegramMethod[Any] = methods[0]
	...
	async def test_login_command_handler(self) -> None:
		methods: list[TelegramMethod[Any]] | None = await self.send_message('/login')
