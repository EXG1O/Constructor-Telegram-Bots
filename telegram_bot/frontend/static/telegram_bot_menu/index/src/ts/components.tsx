...
import { TelegramBotContext } from 'telegram_bot_frontend/components';
...
export function TelegramBotCardFooter(): React.JSX.Element {
	...
	if (telegramBotContext) {
		...
		const handleDeleteButtonClick = (): void => askConfirmModal(
			...
			async (): Promise<void> => {
				...
				if (response.ok) {
					setTimeout(() => window.location.href = personalCabinetUrl, 2000);
				}
