...
export const TelegramBotContext = React.createContext<TelegramBotContextType | null>(null);
...
export function TelegramBotCardHeader(): React.JSX.Element {
...
export function TelegramBotCardBody(): React.JSX.Element {
	const telegramBotContext = React.useContext(TelegramBotContext)!;
	const [telegramBot, setTelegramBot] = [telegramBotContext.telegramBot, telegramBotContext.setTelegramBot];
	const [apiTokenIsEditing, setApiTokenIsEditing] = React.useState<boolean>(false);
									...
									{apiTokenIsEditing ? (
										<>
											<i
												...
												style={{...buttonWithOnlyIconStyle, WebkitTextStroke: '1.4px'}}
											></i>
											<i
												className='bi bi-x-lg text-danger'
												onClick={toggleApiToken}
												style={{...buttonWithOnlyIconStyle, WebkitTextStroke: '1.8px'}}
											></i>
										</>
									) : (
										<i
											...
											onClick={toggleApiToken}
											style={{...buttonWithOnlyIconStyle, WebkitTextStroke: '0.4px'}}
										></i>
									)}
								</div>
							</div>
...
interface TelegramBotCardProps {
	telegramBotInitial: TelegramBot,
	children?: React.JSX.Element,
}

export default function TelegramBotCard({telegramBotInitial, children}: TelegramBotCardProps): React.JSX.Element {
